...
namespace ServiceBase.Authorization
{
    ...
    {
        public static IEnumerable<ScopeAuthorizeAttribute> GetAllScopeAuthorizeAttributes<TController>(Assembly assembly = null, bool fromReferenced = false) 
        {
            ...
            foreach (var typeInfo in ass.DefinedTypes)
            {
                if (typeInfo.IsSubclassOf(ttype) || typeInfo.IsInstanceOfType(ttype))
                {
                    ...
                    {
                        foreach (var attribute in methodInfo.GetCustomAttributes<ScopeAuthorizeAttribute>(true))
                        {
            ...
            if (fromReferenced)
            {
                foreach (var items in ass.GetReferencedAssemblies().Select(s => GetAllScopeAuthorizeAttributes<TController>(Assembly.Load(s))))
                {
        ...
        public static void AddScopePolicies<TController>(this AuthorizationOptions options, string issuer)
        {
            foreach (var attr in GetAllScopeAuthorizeAttributes<TController>())
            {
