...
namespace ServiceBase.Plugins
{
    ...
    public static class IFrontendOptionsServiceExtensions
    {
        ...
        public static async
            Task<IDictionary<string, IDictionary<string, object>>>
            ...
            )
        {
            ...
            {
                IFrontendOptions options =
                    await service.GetFrontendOptionsAsync();

                Type type = options.GetType();

                string name = type.Name
                    .Replace("FrontendOptions", string.Empty);

                if (!whiteList.Any(c => c.Equals(
                    name,
