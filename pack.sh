...
for PROJECT in "${PROJECTS[@]}"
do
mono /usr/local/bin/nuget.exe push ./artifacts/packages/$PROJECT.$VERSION.nupkg -SkipDuplicate -ApiKey $NUGET_API_KEY -Source https://api.nuget.org/v3/index.json
done
